  ...
  return (
    ...
    justify-between px-6 md:px-16 lg:px-36 py-5">
      ...
      <Link to="/" className="max-md:flex-1">
        <img src={assets.logo} alt="CineShow Logo" className="w-36 h-auto" />
      </Link>
      ...
      <div className="flex items-center gap-8">
        <SearchIcon className="max-md:hidden w-8 h-8 text-white cursor-pointer" />
        {!user ? (
          <button onClick={() => openSignIn()} className="px-4 py-1 sm:px-7 sm:py-2 bg-sky-900
        hover:bg-sky-700 transition rounded-full font-medium cursor-pointer">
            Login
          </button>
        ) : (
        <UserButton>
          <UserButton.MenuItems>
            <UserButton.Action label="My Bookings" labelIcon={<TicketPlus/>} onClick={() => {navigate('/myBookings ')}}/>
